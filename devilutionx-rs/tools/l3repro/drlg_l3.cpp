...
void GenerateLevel(lvl_entry entry)
{
		...
		fprintf(stderr, "[rng] after warp %u\n", GetLCGEngineState());
	{
		...
		fprintf(stderr, "[chk] after warp %u\n", h);
	}
		...
		fprintf(stderr, "[rng] after isle %u\n", GetLCGEngineState());
	{
		uint32_t h = 2166136261u;
		for (int j = 0; j < DMAXY; j++) for (int i = 0; i < DMAXX; i++) { h ^= dungeon[i][j]; h *= 16777619u; }
		fprintf(stderr, "[chk] after isle %u\n", h);
	}

		HallOfHeroes();
		River();
		fprintf(stderr, "[rng] after river %u\n", GetLCGEngineState());
	{
		uint32_t h = 2166136261u;
		for (int j = 0; j < DMAXY; j++) for (int i = 0; i < DMAXX; i++) { h ^= dungeon[i][j]; h *= 16777619u; }
		fprintf(stderr, "[chk] after river %u\n", h);
	}
		...
		DRLG_PlaceThemeRooms(5, 10, 7, 0, false);
		{
			...
			if (getenv("DVL_DUMP_AFTER_THEMES")) {
				for (int j = 0; j < DMAXY; j++) { for (int i = 0; i < DMAXX; i++) fprintf(stdout, "%d%c", dungeon[i][j], i==DMAXX-1?'\n':' '); }
				return;
			}
		}
		Fence();
		fprintf(stderr, "[rng] after fence %u\n", GetLCGEngineState());
		if (getenv("DVL_DUMP_AFTER_FENCE")) {
			for (int j = 0; j < DMAXY; j++) { for (int i = 0; i < DMAXX; i++) fprintf(stdout, "%d%c", dungeon[i][j], i==DMAXX-1?'\n':' '); }
			return;
		}
	{
		uint32_t h = 2166136261u;
		for (int j = 0; j < DMAXY; j++) for (int i = 0; i < DMAXX; i++) { h ^= dungeon[i][j]; h *= 16777619u; }
		fprintf(stderr, "[chk] after fence %u\n", h);
	}
		...
		fprintf(stderr, "[rng] after 1x1 %u\n", GetLCGEngineState());
	{
		uint32_t h = 2166136261u;
		for (int j = 0; j < DMAXY; j++) for (int i = 0; i < DMAXX; i++) { h ^= dungeon[i][j]; h *= 16777619u; }
		fprintf(stderr, "[chk] after final %u\n", h);
	}
	}
