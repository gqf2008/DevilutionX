...
std::unique_ptr<uint16_t[]> LoadFileInMem<uint16_t>(const char *path)
{
	...
	const std::string prefix3 = "levels\\l3data\\";
	if (resolved.rfind(prefix3, 0) == 0)
		resolved = "test/fixtures/levels/l3data/" + resolved.substr(prefix3.size());
	const std::string prefix4 = "levels\\l4data\\";
	if (resolved.rfind(prefix4, 0) == 0)
		resolved = "test/fixtures/levels/l4data/" + resolved.substr(prefix4.size());
...
int main(int argc, char **argv)
{
	...
	if (argc > 3)
		warlord = std::atoi(argv[3]);
	if (argc > 4)
		diablo = std::atoi(argv[4]);
	if (argc > 5)
		betrayer = std::atoi(argv[5]);
	if (argc > 6)
		multiplayer = std::atoi(argv[6]);

	for (auto &o : LevelSeeds)
		o = std::nullopt;
	currlevel = level;
	leveltype = DTYPE_HELL;
	gbIsMultiplayer = multiplayer != 0;
	for (auto &q : Quests) {
		q._qactive = QUEST_NOTAVAIL;
		q._qlevel = 0;
		q._qidx = Q_WARLORD;
	}
	Quests[Q_WARLORD]._qidx = Q_WARLORD;
	Quests[Q_WARLORD]._qlevel = 13;
	Quests[Q_WARLORD]._qactive = warlord ? QUEST_INIT : QUEST_NOTAVAIL;
	Quests[Q_DIABLO]._qidx = Q_DIABLO;
	Quests[Q_DIABLO]._qlevel = 15;
	Quests[Q_DIABLO]._qactive = diablo ? QUEST_INIT : QUEST_NOTAVAIL;
	Quests[Q_BETRAYER]._qidx = Q_BETRAYER;
	Quests[Q_BETRAYER]._qlevel = 15;
	Quests[Q_BETRAYER]._qactive = betrayer ? QUEST_ACTIVE : QUEST_NOTAVAIL;
	...
	CreateL4Dungeon(seed, ENTRY_MAIN);
